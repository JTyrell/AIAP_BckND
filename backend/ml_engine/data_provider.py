...
class FileDataProvider(DataProvider):
    ...
    """
    def __init__(self, raw_dir: str = Config.DATASETS_DIR):
...
class DBDataProvider(DataProvider):
    """
    Ingests data from PostgreSQL database and aggregates it into the daily metric format.
    """
    def __init__(self):
        from sqlalchemy import create_engine
        self.engine = create_engine(Config.DATABASE_URL) if Config.DATABASE_URL else None
    ...
    def get_data(self, days: Optional[int] = None) -> pd.DataFrame:
        ...
        if not self.engine:
            print("DBDataProvider: DATABASE_URL not set, falling back to FileDataProvider")
            return FileDataProvider().get_data(days=days)

        print("DBDataProvider: Fetching data from database...")
        # Note: In a production environment, you would push this aggregation logic to SQL for efficiency.
        # For simplicity and parity with FileDataProvider, we fetch raw tables and use the exact same logic.
        try:
            transactions = pd.read_sql_table('transactions', self.engine)
            logs = pd.read_sql_table('operational_logs', self.engine)
            cash = pd.read_sql_table('cash_status', self.engine)
            maint = pd.read_sql_table('maintenance_records', self.engine)
        except Exception as e:
            print(f"DBDataProvider: Error fetching from database: {e}")
            print("DBDataProvider: Falling back to FileDataProvider")
            return FileDataProvider().get_data(days=days)

        # Standardize columns to match CSV format
        if 'withdrawal_amount' in transactions.columns and 'amount' not in transactions.columns:
            transactions.rename(columns={'withdrawal_amount': 'amount'}, inplace=True)
            
        # Add required columns for aggregation logic if missing
        if 'transaction_type' not in transactions.columns:
            transactions['transaction_type'] = 'withdrawal'
            
        # ── Replicate FileDataProvider aggregation exactly ──────────────────
        transactions['record_date'] = pd.to_datetime(transactions['transaction_time']).dt.normalize()
        logs['record_date'] = pd.to_datetime(logs['timestamp']).dt.normalize()
        cash['record_date'] = pd.to_datetime(cash['timestamp']).dt.normalize()
        
        # 1. Transactions daily aggregation (match FileDataProvider)
        amt_col = 'amount'
        if 'transaction_type' in transactions.columns:
            w_mask = transactions['transaction_type'].str.lower().isin(['withdrawal', 'w'])
            d_mask = transactions['transaction_type'].str.lower().isin(['deposit', 'd'])
        ...
        # 2. Errors & Uptime per day (match FileDataProvider)
        errors_daily = logs[logs['uptime_status'] == 0].groupby(
            ['atm_id', 'record_date']
        ).size().reset_index(name='error_count')
        
        downtime_daily = logs.groupby(
            ['atm_id', 'record_date']
        )['downtime_duration'].sum().reset_index(name='downtime_duration')

        # 3. Cash Status — Starting/Ending (match FileDataProvider)
        cash = cash.rename(columns={'remaining_cash': 'ending_cash_balance'})
        # Drop cash_id and timestamp if present
        cash = cash.drop(columns=[c for c in ['cash_id', 'timestamp'] if c in cash.columns])
        ...
        # Merge base table (match FileDataProvider order)
        daily = cash_daily.copy()
        if not daily.empty:
            daily = pd.merge(daily, tx_daily, on=['atm_id', 'record_date'], how='left')
            daily = pd.merge(daily, errors_daily, on=['atm_id', 'record_date'], how='left')
            ...
            # Uptime calculation (match FileDataProvider)
            daily['uptime_minutes'] = 1440 - daily['downtime_duration'].clip(0, 1440)
            daily['uptime_percentage'] = (daily['uptime_minutes'] / 1440) * 100

            # 4. Integrate service_date from maintenance logs
            maint['record_date'] = pd.to_datetime(maint['maintenance_date']).dt.normalize()
            m_latest = maint.sort_values('record_date').drop_duplicates('atm_id', keep='last')
            m_latest = m_latest[['atm_id', 'record_date']].rename(columns={'record_date': 'service_date'})
            daily = pd.merge(daily, m_latest, on='atm_id', how='left')
...
def get_data_provider() -> DataProvider:
    if getattr(Config, 'DATABASE_URL', ''):
        # DBDataProvider will automatically test the connection and fall back to FileDataProvider if it fails
